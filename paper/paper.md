...
-  sample and probe simulation converted results in VTK format with the VTK library.
...
-  control and modify HEC-RAS 2D simulations through the `HEC_RAS_Model` class. The control is through the Python library pywin32 [@hammond2000]. `pyHMT2D` interfaces with HEC-RAS through the Component Object Model (COM), which is the foundation for Microsoft's OLE and ActiveX technologies. 
...
To demonstrate the automatic calibration functionality, Fig. 3 shows a simple, yet complete, example which is the water surface profile in a 10 km long river. This example is located in the "examples/calibration" directory of the repository on GitHub. The channel is divided into to segments, each of which has their own calibration parameter (the Manning's roughness $n$ in this case). Fig. 3(a) shows the layout of the case and the simulated profile using the simple `Backwater-1D` class. This result is used as a manufactured solution. Four sampling points are shown in Fig. 1(a), which are used to sample results as measurement data. Then the `Calibrator` is constructed and the calibration uses `scipy.optimize`'s "Nelder-Mead" method. Fig. 3 (b) shows the trajectory of the calibration in the parameter space and Fig. 3(c) shows the calibration error as a function of iteration number. At the end, the calibration succeed with 26 iterations and the two calibration parameters $n_1$ and $n_2$ are very close to the true values. 
...
-  conversion of SRH-2D model mesh and result to 3D through extrusion (one layer or multiple layers) and
   VTK interpolation. It can be done through the `SRH_2D_Data` class. This feature is useful if one wants to use 2D simulation result in 3D applications, e.g., fish passage design or use 2D result as initial condition for 3D simulations. Currently, conversion
