...
![Package structure and scheme diagram for `pyHMT2D`.](./figures/package_scheme_diagram.png)
...
-  conversion of SRH-2D mesh and result to 3D through vertical extrusion (one layer or multiple layers) and
...
`pyHMT2D`'s  core functionality relies on the following Python packages: h5py [@collette2013], VTK [@vtkbook2006], GDAL [@gdal2020], NumPy [@harris2020], and Matplotlib [@hunter2007]. Optionally, pywin32 [@hammond2000] is needed to control the simulation of HEC-RAS and affine [@Gillies2014] is needed to read HEC-RAS results. 
