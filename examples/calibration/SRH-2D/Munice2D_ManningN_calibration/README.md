...
left_2 (zone 4) in the Muncie 2D case using pyHMT2D's `SRH_2D_Model` and `SRH_2D_Data`
classes. The calibration uses high water marks from `HWMs.dat`.
...
- Run calibration with `gp_minimize`:
  ...
  - Modify Manning's n in the SRH-2D material file using `SRH_2D_Data`.
  - Run SRH-2D via `SRH_2D_Model`.
  - Convert results to VTK using `SRH_2D_Data`.
...
The script uses pyHMT2D's `SRH_2D_Model`, `SRH_2D_Data`, and `vtkHandler`.
